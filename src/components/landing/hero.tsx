      ...
      <div className="mx-auto grid w-full max-w-6xl items-end gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="reveal max-w-3xl space-y-7">
          ...
          <div className="grid max-w-2xl gap-3 pt-2 sm:grid-cols-3">
            <div className="rounded-xl border border-slate-200/20 bg-slate-100/5 px-3 py-2.5">
              <p className="kicker">Scale</p>
              <p className="mt-1 text-xl font-bold text-white">10,000</p>
              <p className="text-xs text-slate-400">NFT output / run</p>
            </div>
            <div className="rounded-xl border border-slate-200/20 bg-slate-100/5 px-3 py-2.5">
              <p className="kicker">Latency</p>
              <p className="mt-1 text-xl font-bold text-white">&lt; 7 min</p>
              <p className="text-xs text-slate-400">collection blueprint</p>
            </div>
            <div className="rounded-xl border border-slate-200/20 bg-slate-100/5 px-3 py-2.5">
              <p className="kicker">Output</p>
              <p className="mt-1 text-xl font-bold text-white">1 export</p>
              <p className="text-xs text-slate-400">contract + metadata + site</p>
            </div>
          </div>
        </div>

        <aside className="reveal panel w-full rounded-2xl p-5 text-white">
          <p className="kicker">Project command center</p>
          <h2 className="mt-1.5 text-lg font-bold">Cyberpunk Foxes Season I</h2>
          <p className="mt-1.5 text-xs text-slate-300">Primary brief parsed, constraints validated, generation queued.</p>
