...
export function buildERC721AContract(input: ContractTemplateInput) {
...
contract AINFTCollection is ERC721A, Ownable, ERC2981, Pausable {
    uint256 public immutable maxSupply = ${input.maxSupply};
    uint256 public mintPrice = ${input.mintPriceWei};
    ...
    constructor(address royaltyReceiver) ERC721A("${input.collectionName}", "${input.symbol}") Ownable(msg.sender) {
        _setDefaultRoyalty(royaltyReceiver, ${input.royaltyBps});
    }
